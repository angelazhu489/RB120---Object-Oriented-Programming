=begin
COMMUNITY LIBRARY - On line 42 of our code, we intend to display 
information regarding the books currently checked in to our community 
library. Instead, an exception is raised. Determine what caused this 
error and fix the code so that the data is displayed as expected.
=end
class Library
...
community_library.display_data # HERE

=begin
ANIMAL KINGDOM - The code below raises an exception. Examine the error 
...
=begin
WISH YOU WERE HERE - On lines 37 and 38 of our code, we can see that 
grace and ada are located at the same coordinates. So why does line 
39 output false? Fix the code to produce the expected output.
=end
class Person
  attr_reader :name
...
=begin
EMPLOYEE MANAGEMENT - We have written some code for a simple employee 
management system. Each employee must have a unique serial number. 
...
=begin
FILES - You started writing a very basic class for handling files. 
However, when you begin to write some simple test code, you get a 
NameError. The error message complains of an uninitialized constant 
File::FORMAT.
...
=begin
SORTING DISTANCES - When attempting to sort an array of various 
lengths, we are surprised to see that an ArgumentError is raised. 
Make the necessary changes to our code so that the various lengths 
can be properly sorted and line 62 produces the expected output.
=end
class Length
...
=begin
BANK BALANCE - We created a simple BankAccount class with overdraft 
protection, that does not allow a withdrawal greater than the amount 
...
Why are we seeing this unexpected output? Make changes to the code so 
that we see the appropriate behavior.
=end
class BankAccount
  attr_reader :balance
...
=begin
TASK MANAGER - Valentina is using a new task manager program she wrote. 
...
=begin
YOU'VE GOT MAIL! - Can you decipher and fix the error that the 
...
=begin
DOES IT ROCK OR NOT? - In order to test the case when authentication 
fails, we can simply set API_KEY to any string other than the correct 
key. Now, when using a wrong API key, we want our mock search engine 
to raise an AuthenticationError, and we want the find_out method to 
catch this error and print its error message API key is not valid.
...
module DoesItRock
  ...
  class NoScore < AuthenticationError; end
